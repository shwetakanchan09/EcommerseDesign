package com.shweta.reyound.activity;

import androidx.appcompat.app.AppCompatActivity;

import android.app.Activity;
import android.content.Intent;
import android.os.Bundle;
import android.view.View;
import android.widget.AdapterView;
import android.widget.GridView;
import android.widget.ImageView;
import android.widget.LinearLayout;
import android.widget.Toast;

import com.shweta.reyound.R;
import com.shweta.reyound.adapter.JeansAdapter;
import com.shweta.reyound.adapter.MakeupAdapter;

public class MakeupActivity extends Activity {

    Intent intent;
    LinearLayout ll_Relevant,ll_Filter;
    ImageView backbtn;

    GridView grid;
    String[] web = {
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup",
            "Beauty Products - Makeup"



    } ;
    int[] imageId = {
            R.drawable.make1,
            R.drawable.make2,
            R.drawable.make3,
            R.drawable.make4,
            R.drawable.make5,
            R.drawable.make6,
            R.drawable.make7,
            R.drawable.make8,
            R.drawable.make9,
            R.drawable.make10,
            R.drawable.make1,
            R.drawable.make2,
            R.drawable.make3,
            R.drawable.make4,
            R.drawable.make5,
            R.drawable.make6,
            R.drawable.make7,
            R.drawable.make8,
            R.drawable.make9,
            R.drawable.make10
    };



    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_makeup);


        ll_Relevant = findViewById(R.id.ll_Relevent);
        ll_Filter = findViewById(R.id.ll_Filter);
        backbtn = findViewById(R.id.back_btn);

        backbtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                finish();
            }
        });


        ll_Relevant.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                intent = new Intent(getApplicationContext(), ReleventActivity.class);
                startActivity(intent);
            }
        });

        ll_Filter.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                intent = new Intent(getApplicationContext(), FilterActivity.class);
                startActivity(intent);
            }
        });

        MakeupAdapter adapter = new MakeupAdapter(MakeupActivity.this, web, imageId);
        grid=(GridView)findViewById(R.id.grid);
        grid.setAdapter(adapter);
        grid.setOnItemClickListener(new AdapterView.OnItemClickListener() {

            @Override
            public void onItemClick(AdapterView<?> parent, View view,
                                    int position, long id) {
                Toast.makeText(MakeupActivity.this, "You Clicked at " +web[+ position], Toast.LENGTH_SHORT).show();

            }
        });
    }
}